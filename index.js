...
function flatArrayAndCalculateMean(arr) {
  ...
  // Use the mean() function from lodash to calculate the mean of the array
  let flatArray = _.flatten(arr);
  let result = _.mean(flatArray);
  return result;
...
function sortByNamesAndCapitalizeFirstLetter(arr) {
  // Use the sortBy() function from lodash to sort the array by name
  let sortedArray = _.sortBy(arr, [function(o) { return o.name; }]);  
  let result = sortedArray.map((elem) => _.capitalize(elem.name)); 
...
function transformToSnakeCaseAndAllCaps(arr) {
  ...
  let snakeCaseArray = arr.map((elem) => _.snakeCase(elem));
  let result = snakeCaseArray.map((elem) => _.toUpper(elem));
  return result;
